...
class Skill_Damage(Damage_Calc):
    def __init__(self, Potency: int, Trait: int) -> None:
        ...
        self.WD: int = int(self.Status.WD_Damage())
        self.TNC: int = int(self.Status.TNC_Damage())
        self.Trait: int = int(Trait)

    def D1(self) -> list:
        D1_Result: float = Decimal(self.Potency * self.ATK * self.DET / 100 / 1000)
        D1_Final_Result: int = math.floor(D1_Result)
        return D1_Final_Result

    def D2(self, D1: float) -> float:
